...
版本：v0.1｜状态：M0 / M1 / M2 / M3 编码已完成｜日期：2026-09-21。
...
CueKB是一套支持中文、英语等多语言文档与查询的证据检索服务。“线索知识库”是中文名称，不限制支持的语言范围。输入自然语言问题与可信访问身份，输出排序后的原文证据、适用条件、来源定位及本次检索状态。快速、准确高于图谱规模或拟脑程度。
...
**待开始**：证据状态评估、质量评测工具和运行指标。

**待确认**：文档级ACL、外部平台同步及更复杂查询。DOC旧格式、复杂图形理解、图片/视频语义检索、全自动知识图谱和生产故障动作执行不在当前范围。
...
应用采用模块化结构，API与独立Worker共享领域模型；解析器是Worker调用的库，不要求另建解析微服务。Embedding和重排均使用项目外部的独立服务地址；Embedding配置完成后才接受导入/检索，重排地址为空时不调用重排。CueKB不构建、运行或保存任一模型的权重。CueKB不集成生成式LLM；是否将检索结果交给LLM以及如何生成回答，完全由外部系统决定。
...
| 关系DDL | 实体、别名、mention、关系和证据表 | 同知识库与证据约束 | **已完成** | `0003_relations_generations.py` |
| 关系写入与一跳扩展 | 管理员维护有出处的关系 | `related`有界一跳扩展 | **已完成** | `adapters/knowledge.py` |
| 完整上下文组装 | 章节层级与表头保留 | 有预算地补章节、相邻步骤和单位 | **已完成** | `services/context.py` |
...
开发模式读取修订号只是返回字段，不执行完整事务快照比较。`auto`已按确定性标识符规则选择exact或hybrid，但内存模式没有模型，hybrid会显式标记向量降级。内存开发适配器不保存关系；生产`related`执行有证据的一跳扩展。
...
| 5 | 重排适配器、检索编排、PG正文 | 配置重排地址时，query＋有限候选正文→可选重排；否则保留RRF顺序 | 重排不生成答案；context按预算组装可追溯原文 |
...
查询阶段不再运行Docling/OCR；Worker不在每个在线请求中执行。OpenSearch先找候选，PG最终决定哪些候选可以返回。关键词、向量和重排各自只承担检索职责。关系扩展和有预算的完整上下文代码状态为**已完成**；生成式回答由外部系统负责，不纳入CueKB范围（D19）。
...
| sections完整层级写入 | 写入标题路径层级并关联chunks；迁移回填历史块 | **已完成** |
...
| entities / entity_aliases / mentions | 实体、审核别名及原文块定位 | **已完成** |
| relation_assertions / relation_evidence | 有出处关系、条件、支持或反驳证据 | **已完成** |
| index_generations | 模型、维度、分块版本、加密配置、重建与切换记录 | **已完成** |
...
章节层级、父章节及相邻步骤补充、表头/单位保留和总上下文预算的代码状态为**已完成**。`context`按原文顺序拼接，`context_parts`提供每块锚点，`context_truncated`说明预算限制。每命中最多8块、4000字符，总计12000字符；优先命中本身及近邻，父章节内容超过预算时不会无限展开。表格切分重复已有表头，不推测缺失单位。OCR不确定、阅读顺序错误和表头丢失的高级质量规则代码状态为**待开始**。
...
完整章节层级、关系和审核别名维护代码状态为**已完成**；页覆盖和复杂表格质量规则仍待真实样本推进。关系由管理员显式录入，不从语料自动推断因果。
...
每次检索尝试中的全部权威读取（修订、候选、关系、上下文、权限）共享一个REPEATABLE READ只读事务。该事务结束后再开新事务复验Key/ACL、版本、关系和上下文，避免长快照隐藏已提交撤权。修订变化最多重试一次；持续变更时按最终事务快照过滤并标记index_transition。保证的是最终复验快照时的合法性，不声称响应发出后仍可撤回已发送字节。
...
index generation重建、验证、切换和回退代码状态为**已完成**，细节见8.3。关系查询只读取当前发布且未删除的证据块；删除或版本替换立即使旧证据不可见，关系显式删除级联移除证据引用。硬删除、备份到期清理和保留策略状态为**待确认**。
...
| related | 调用方请求关系查询 | hybrid＋有证据一跳扩展、RRF融合，`scope_limited=true` | **已完成** |
...
7. 返回默认8个、最多20个证据，按预算拼装章节/相邻原文及逐块锚点；`evidence_status`仍为`unassessed`。

审核别名、一跳扩展和上下文字符/块预算代码状态为**已完成**；Tokenizer级截断策略和证据状态评估仍为**待开始**。候选数、deadline和重排门槛是配置值，不代表最佳参数。
...
轻量关系DDL、写入服务、管理接口和一跳查询代码状态为**已完成**。支持的关系包括belongs_to、adjacent_to、alias_of、revises、replaces、references，以及有证据且通过校验的depends_on/applies_to。

相似文本只能作为召回线索，不能自动变为因果/依赖关系。关系扩展必须回到原文，按问题的关系类型、时间和方向筛选，并限制一跳、数量和时间。对“全部影响范围”明确`scope_limited=true`，不得将受限一跳结果标为完整。

起始实体来自调用方`relations.entity_ids`、query与名称/审核别名的精确匹配，以及合法召回块的显式mention。最多20个起点，按UUID稳定裁剪，只走一跳；默认最多30条关系证据，SQL默认100ms。方向、类型、含时区时间和显式型号/软件版本条件同时过滤；带条件的关系在调用方未给出匹配filter时不扩展。不按命中频率自动创建别名，不推断传递关系。支持与反驳证据都按原文返回，`stance`不等于已校准的事实判断。
...
| 关联超时 | SQL局部statement_timeout和savepoint回滚，保留普通合法候选并标记relation_unavailable；预算耗尽标记relation_budget_exhausted |
...
| GET/PUT/DELETE /v1/knowledge-bases/{id}/entities及relations | 实体、关系和证据管理；写入需kb admin | **已完成** |
| GET/POST /v1/index-generations及activate/rollback/DELETE | 系统管理员索引重建、切换和回退 | **已完成** |
...
| scope_limited | related固定true，有界一跳不代表全部影响范围 |
...
限制上传格式/大小与解析资源，拒绝路径穿越和任意远端URL抓取。原文属于不可信输入；外部系统若接入回答模型，应把检索原文作为数据而非指令，不应因文档内容越权调用工具。
...
用户16GB M4机器不能在无实测情况下保证外部模型服务、OpenSearch和开发工具同时满足1秒目标。可选择与CueKB网络隔离部署的质量基线模型服务，或使用覆盖目标语种的轻量Embedding、减少重排候选并单独验收；仅支持中文的模型不能替代多语言需求。严格离线时外部模型与OCR权重需提前下载，禁用远程推理。
...
`index_generations`持久记录物理索引、模型、维度、注册分块版本、加密模型配置、源修订、块数、尝试次数及状态。当前仅注册`structured-v1`（已保存权威块）；重建不重新解析文件，不接受未知分块算法。M3保留版本边界，后续新分块算法需要单独实现和数据迁移。

Worker取得独占session advisory lock后将queued/building任务转入building，分页32块读取当前published及ready正文，重新编码并写独立索引；每页验证可读取的正文，最后验证总块数。失败使用有界退避重试，崩溃后重建同一未激活索引。内容写入和普通入库持有对应共享锁；锁冲突返回409/Worker等待，查询继续使用旧索引。Key吊销与ACL变更不受该维护锁阻塞。

ready后由管理员触发切换。切换重验源配置修订、全部KB内容修订、索引模型/维度/块数，在同一PG事务中更新active_index、Embedding维度及模型配置，并退休旧generation。请求与Worker一次读取匹配的模型/物理索引，避免把查询向量发给不同模型的索引。回退将retired记录的旧配置重新排队，按当前PG内容建立新的generation再切换，不恢复旧权限或旧文档。原索引保留供在途查询；取消和失败索引不自动清理。

迁移0003回填既有章节关联并创建新增表；发布时先停旧API/Worker，迁移完成后启动同一版本的服务。当前策略是单机维护窗口内可靠重建，不宣称零写入停顿或高可用。操作步骤在README、HTTP契约在API_GUIDE维护。
...
轻量关系扩展、完整上下文代码状态为**已完成**；质量评测工具和运行指标为**待开始**；首期管理门户为**已完成**，生成式回答由外部系统自主决定并集成，不纳入CueKB范围。现有M0、M1、M2、M3编码状态均为**已完成**，不为匹配历史拟议目录迁移到`backend/`。

编码顺序：M0环境与版本基线（**已完成**）→M1文档/关键词（**已完成**）→M2混合/重排（**已完成**）→首期管理门户（**已完成**）→M3关系与上下文（**已完成**）→M4质量与运行能力（**待开始**）；生成式回答不纳入CueKB开发阶段（D19）。详细进度只在TASK_BOARD维护。
...
| 图扩展加入大量不相关内容 | M3（**已完成**）采用有证据关系、显式路径、数量/方向/一跳限制与消融 |
