...
| 个人知识库 | 导出的聊天、项目设计、决策记录、笔记 | 本地独立实例；门户状态为**待确认**；模型与解析均本地时可离线 |
...
**待确认**：轻量Web门户、LLM回答、文档级ACL、外部平台同步及更复杂查询。DOC旧格式、复杂图形理解、图片/视频语义检索、全自动知识图谱和生产故障动作执行不在当前范围。
...
flowchart LR
    ...
    RET -->|有限候选相关性排序| RANK["重排适配器：bge-reranker-v2-m3"]
    RET -->|证据JSON| C
```

应用采用模块化结构，API与独立Worker共享领域模型；解析器是Worker调用的库，不要求另建解析微服务。Embedding和重排由适配器隔离，Compose分离API、Worker和模型进程。普通检索不调用生成式LLM。
...
关键词检索是另一条路径：`查询文本→OpenSearch BM25→关键词候选`，无需先生成Embedding。混合检索才把关键词候选和向量候选交给RRF融合，再做有限重排。文档向量与查询向量必须使用一致的模型修订和编码配置；换模型需要重建匹配的向量索引。
...
| 重排 | 不处理入库向量 | 按deadline、路径和负载处理有限候选 | **已完成** | `model_server.py`、`services/retrieval.py` |
...
表中源码短路径均相对于`src/cuekb/`。Compose会运行迁移、API、Worker、模型、PG和OpenSearch；内部数据服务不映射宿主机端口。确切依赖见`pyproject.toml`；不引入Neo4j、Redis、独立向量数据库或分布式消息队列。
...
**已完成的生产路径：候选召回、重排、版本切换重试与最终证据校验。预算及降级见第5节。**
...
| 5 | 重排适配器、检索编排、PG正文 | query＋有限候选正文→可选重排；选top_k | 重排不生成答案；当前context返回命中段落 |
...
| hybrid | 调用方指定的自然语言检索 | BM25与向量并行、RRF、可选重排 | **已完成** |
| related | 调用方请求关系查询 | 当前执行hybrid并返回`scope_limited=true` | hybrid回退**已完成**；一跳扩展**待开始** |
| auto | 调用方默认值 | 确定性正则选择exact或hybrid | **已完成** |

精确查询不只是“识别到了错误码”就跳过其他条件。相同错误码在不同产品存在时，结合明确型号或返回分组和歧义。调用方可通过`mode`明确选择；`auto`的标识符正则、总deadline、模型超时、重排候选数、重排最低剩余预算和重排开关均来自`CUEKB_*`配置。策略由确定性代码执行，但阈值不是散落在业务代码里的不可修改常量。
...
5. 非exact路径在开关启用、候选足够且剩余deadline充足时对有限候选重排。
...
deadline预算、模型调用超时、OpenSearch请求超时、重排剩余预算门禁和模型429过载响应代码状态为**已完成**。覆盖PG及所有子任务的硬截止取消代码状态为**待开始**；当前`search_deadline_ms`用于模型超时与重排决策，不应解释为整个HTTP请求的强制中止时刻。
...
模型服务安全控制代码状态为**已完成**：只从运维配置中的固定仓库和完整commit revision下载，不接受检索或导入API传入模型路径；容器使用非root用户；冻结依赖经`pip-audit`未发现已知漏洞。
...
Compose部署代码状态为**已完成**：包含迁移门禁、API、Worker、模型服务、PostgreSQL、OpenSearch及持久卷；生产配置强制密钥和固定模型修订，API不会回退内存。步骤见[README](../README.md)。
...
搜索响应的query编码、关键词/向量搜索、重排和总耗时，以及执行/跳过阶段和降级原因，代码状态为**已完成**。持久化指标、关系/上下文耗时、任务积压、模型队列、发布延迟、告警和运行看板代码状态为**待开始**。日志默认隐藏完整正文和凭据。
...
轻量关系扩展、完整上下文、质量评测工具和运行指标代码状态为**待开始**；门户与生成式回答代码状态为**待确认**。现有M0、M1、M2编码状态均为**已完成**，不为匹配历史拟议目录迁移到`backend/`。

编码顺序：M0环境与版本基线（**已完成**）→M1文档/关键词（**已完成**）→M2混合/重排（**已完成**）→M3关系与上下文（**待开始**）→M4质量与运行能力（**待开始**）→M5门户与可选回答（**待确认**）。详细进度只在TASK_BOARD维护。
