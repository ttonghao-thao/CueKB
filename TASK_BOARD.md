...
| Docker目标环境启动 | **待开始** | 当前环境无Docker CLI；OpenSearch 3启动需设置`CUEKB_OPENSEARCH_INITIAL_ADMIN_PASSWORD`，随后在Linux目标环境重新`docker build`两个镜像，核对镜像大小和Worker的`torch.version.cuda is None`，再执行Compose迁移/启动、门户模型配置、生产验收脚本及门户浏览器检查 |
